...
void ChessBoard(int **Board, int tr, int tc, int dr, int dc, int size) {
    ...
    if (dr < tr+s && dc < tc + s) {  // 特殊方格在左上角子棋盘
        ...
        ChessBoard(Board, tr, tc, tr + s - 1, tc + s - 1, s); // 覆盖左上角子棋盘内其余方格 （tr+s-1, tc+s）就是上一行覆盖的那块
    }
    ...
    if(dr < tr+s && dc >= tc+s) {   // 特殊方格在右上角子棋盘
        ...
        ChessBoard(Board, tr, tc + s, tr + s - 1, tc + s, s); // 覆盖右上角子棋盘内其余方格
    }
    ...
    if(dr >= tr+s && dc < tc+s) {   // 特殊方格在左下角子棋盘
        ...
        ChessBoard(Board, tr + s, tc, tr + s, tc + s - 1, s); // 覆盖左下角子棋盘内的其余方格
    }
    ...
    if(dr >= tr+s && dc >= tc+s){  // 特殊方格在右下角子棋盘
        ...
        ChessBoard(Board, tr + s, tc + s, tr + s, tc + s, s); // 覆盖右下角子棋盘内的其余方格
    }
}

int main() {
    int n = 0;
    cout << "please tell me the chess board's size (n): ";
    cin >> n;
    int **Board = new int*[n];
    for (int i = 0; i < n; i++)
        Board[i] = new int[n]; //开辟列
